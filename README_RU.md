...
Keycloak в `sso/` — это пример настройки с преднастроенными тестовыми пользователями и кастомной темой страницы входа. **Homni работает с любым OIDC/OAuth провайдером** &mdash; Authentik, Auth0, Okta, Google Workspace или любым другим провайдером, поддерживающим OpenID Connect.
