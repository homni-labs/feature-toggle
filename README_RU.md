...
Большинство решений для фича-флагов — это SaaS с оплатой за пользователя и без нормального контроля доступа. Togli идёт другим путём:
...
- &#x2615; **[Java SDK](#sdk)** &mdash; zero-dependency клиентская библиотека с `isEnabled()`, `evaluate()`, proxy-роутинг интерфейсов, фоновый кэш
...
| SDK | Язык | Статус | Пакет |
|-----|------|--------|-------|
| [Java SDK](sdk/java/README_RU.md) | Java 21+ | Доступен | [GitHub Packages](https://github.com/homni-labs/feature-toggle/packages) |
...
// Fallback: тогл ON → новая логика, тогл OFF → старая
...
Togli идёт с полным стеком мониторинга из коробки. Всё просто работает &mdash; просто `docker compose up` и откройте Grafana.
...
Всё работает из коробки — для локальной разработки настройка не нужна.
...
Конфигурация фронтенда загружается из `/config.json` при старте. Работает из коробки — ручная настройка не нужна.
...
- [ ] Процентный раскат &mdash; включить тогл для N% запросов с consistent hashing
- [ ] Snapshot & restore &mdash; сохранить состояние тогглов как снимок, откат одним кликом
- [ ] Аудит-лог &mdash; отслеживание всех действий с timeline-лентой на каждом тогле
