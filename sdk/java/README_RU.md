...
- **Zero dependencies** &mdash; только `java.*` импорты, Java 21+
...
// Простая проверка — используется default environment
...
// Fallback — разная логика в зависимости от состояния тогла
...
SDK держит тогглы актуальными, периодически опрашивая бэкенд (по умолчанию: каждые 60 минут) и автоматически подхватывает все изменения:
...
|-------|-----------|----------|
| `isEnabled(name)` | `boolean` | Проверить тогл в default environment. Возвращает `false` при любой ошибке |
| `isEnabled(name, env)` | `boolean` | Проверить тогл в явном окружении. Возвращает `false` при любой ошибке |
| `toggle(name)` | `Toggle` | Получить полные данные тогла. Бросает `TogliNotFoundException` если не найден |
| `allToggles()` | `List<Toggle>` | Все тогглы проекта |
| `allEnvironments()` | `List<EnvironmentInfo>` | Все окружения проекта |
| `projectInfo()` | `ProjectInfo` | Метаданные проекта (id, name, slug, archived) |
| `evaluate(name, Runnable, Runnable)` | `void` | Выполнить enabled/disabled действие по состоянию тогла |
| `evaluate(name, Supplier, Supplier)` | `<T>` | Вернуть enabled/disabled значение по состоянию тогла |
| `proxy(Class, enabled, disabled)` | `<T>` | Создать proxy интерфейса с `@FeatureToggle` роутингом |
| `refresh()` | `void` | Принудительное обновление кэша |

Все методы `evaluate` и `proxy` также имеют перегрузки с явным указанием окружения.

---

## 🛡 Обработка ошибок

`isEnabled()` **никогда не бросает исключений** и **никогда не роняет** ваше приложение. При любой ошибке (сбой сети, сервер 500, таймаут) тихо возвращает `false`.
